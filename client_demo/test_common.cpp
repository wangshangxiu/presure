 ...
 */
void im_hmac_sha256_cleanup(void *hmac_context, void *user_data)
{
	if (hmac_context) {
...
int im_hmac_sha256_final(void *hmac_context, signal_buffer **output, void *user_data)
{
	int result = 0;
...
int im_hmac_sha256_update(void *hmac_context, const uint8_t *data, size_t data_len, void *user_data)
{
	HMAC_CTX *ctx = (HMAC_CTX*)hmac_context;
	int result = HMAC_Update(ctx, data, data_len);
	return (result == 1) ? 0 : -1;
}
int im_hmac_sha256_init(void **hmac_context, const uint8_t *key, size_t key_len, void *user_data)
{
...
int im_random_generator(uint8_t *data, size_t len, void *user_data)
{
...
int im_sha512_digest_init(void **digest_context, void *user_data)
{
	int result = 0;
	EVP_MD_CTX *ctx;
...
int im_sha512_digest_update(void *digest_context, const uint8_t *data, size_t data_len, void *user_data)
{
...
int im_sha512_digest_final(void *digest_context, signal_buffer **output, void *user_data)
{
	int result = 0;
	unsigned char md[EVP_MAX_MD_SIZE];
	unsigned int len = 0;
...
void im_sha512_digest_cleanup(void *digest_context, void *user_data)
{
	EVP_MD_CTX *ctx = (EVP_MD_CTX *)digest_context;
	EVP_MD_CTX_destroy(ctx);
}
...
void im_libsignal_log(int level, const char *message, size_t len, void *user_data)
{
	switch (level) {
	case SG_LOG_ERROR:
...
int im_aes_encrypt(signal_buffer **output,
	int cipher,
...
int im_aes_decrypt(signal_buffer **output,
	int cipher,
	const uint8_t *key, size_t key_len,
	const uint8_t *iv, size_t iv_len,
...
void im_setup_crypto_provider(signal_context *context)
{
	signal_crypto_provider provider = {
		.random_func = im_random_generator,
		.hmac_sha256_init_func = im_hmac_sha256_init,
		.hmac_sha256_update_func = im_hmac_sha256_update,
		.hmac_sha256_final_func = im_hmac_sha256_final,
		.hmac_sha256_cleanup_func = im_hmac_sha256_cleanup,
		.sha512_digest_init_func = im_sha512_digest_init,
		.sha512_digest_update_func = im_sha512_digest_update,
		.sha512_digest_final_func = im_sha512_digest_final,
		.sha512_digest_cleanup_func = im_sha512_digest_cleanup,
		.encrypt_func = im_aes_encrypt,
		.decrypt_func = im_aes_decrypt,
		.user_data = 0
	};
